...
public class CollisionManager
{
    ...
    private static Dictionary<GameObject, ColliderPrefab> _go2ColPrefab =
        new Dictionary<GameObject, ColliderPrefab>();

    private static Dictionary<GameObject, int> _go2Layer = new Dictionary<GameObject, int>();

    private static Dictionary<ILPTriggerEventHandler, ColliderProxy> _mono2ColProxy =
        new Dictionary<ILPTriggerEventHandler, ColliderProxy>();

    private static Dictionary<ColliderProxy, ILPTriggerEventHandler> _colProxy2Mono =
    ...
    public LFloat parcent = new LFloat(true, 100);
    ...
    private int[] allTypes = new[] { 0, 1, 2 };
    ...
    private void GlobalOnTriggerEvent(ColliderProxy a, ColliderProxy b, ECollisionEvent type)
    {
    ...
    public void RegisterPrefab(GameObject go, int val)
    {
        _go2Layer[go] = val;
    }
    ...
    public void RegisterEntity(GameObject fab, GameObject obj, BaseEntity entity)
    {
        ...
        {
            prefab = CollisionSystem.CreateColliderPrefab(fab.GetComponent<ColliderData>());
        }

        AttachToColSystem(_go2Layer[fab], prefab, obj, entity);
    }

    private void AttachToColSystem(int layer, ColliderPrefab prefab, GameObject gameObject, BaseEntity entity)
    {
