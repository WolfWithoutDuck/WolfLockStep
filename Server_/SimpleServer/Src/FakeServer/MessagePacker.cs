    ...
    public object DeserializeFrom(ushort opcode, byte[] bytes, int index, int count)
    {
        ...
        switch (type)
        {
            case EMsgType.JoinRoom:
                return BaseFormater.FromBytes<Msg_JoinRoom>(bytes, index, count);
            case EMsgType.QuitRoom:
                return BaseFormater.FromBytes<Msg_QuitRoom>(bytes, index, count);
            case EMsgType.PlayerInput:
                return BaseFormater.FromBytes<Msg_PlayerInput>(bytes, index, count);
            case EMsgType.FrameInput:
                return BaseFormater.FromBytes<Msg_FrameInput>(bytes, index, count);
            case EMsgType.StartGame:
                return BaseFormater.FromBytes<Msg_StartGame>(bytes, index, count);
